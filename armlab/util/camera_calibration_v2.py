...
if __name__ == '__main__':
    ...
    np.savetxt('intrinsic_matrix.txt',camera_matrix,delimiter=',')
    np.savetxt('extrinsic_rvec.txt',rvecs[-1],delimiter=',')
    np.savetxt('extrinsic_tvec.txt',tvecs[-1],delimiter=',')
    ...
    while True:
        rgb_frame = cv2.cvtColor(freenect.sync_get_video()[0],cv2.COLOR_RGB2BGR)
        undistorted = cv2.undistort(rgb_frame, camera_matrix, dist_coefs, None, camera_matrix)
        frames_str = "Showing undistorted image (press ESC)" + str(frame_count)
        cv2.putText(rgb_frame, frames_str, (50,50), font, 1, (255,255,255),2, cv2.LINE_AA)   
        mean_error = 0
        
        imgpoints2, _ = cv2.projectPoints(obj_points[-1], rvecs[-1], tvecs[-1], camera_matrix, dist_coefs)
        #IPython.embed()
        for each in imgpoints2:
            cv2.circle(rgb_frame,(each[0][0],each[0][1]),2,(0,0,255),2)

        imgpoints2, _ = cv2.projectPoints(np.array([[482.6,-406.4,0]]), rvecs[-1], tvecs[-1], camera_matrix, dist_coefs)
        for each in imgpoints2:
            cv2.circle(rgb_frame,(np.int16(each[0][0]),np.int16(each[0][1])),2,(0,0,255),2)
            linePtr1=(np.int16(each[0][0]),np.int16(each[0][1]))    
        imgpoints2, _ = cv2.projectPoints(np.array([[482.6,-406.4,-115.18]]), rvecs[-1], tvecs[-1], camera_matrix, dist_coefs)
        for each in imgpoints2:
            cv2.circle(rgb_frame,(np.int16(each[0][0]),np.int16(each[0][1])),2,(0,0,255),2)
            linePtr2=(np.int16(each[0][0]),np.int16(each[0][1]))
        cv2.line(rgb_frame,linePtr1,linePtr2,(0,255,0),2)
        
        imgpoints2, _ = cv2.projectPoints(np.array([[482.6,-368.1,0]]), rvecs[-1], tvecs[-1], camera_matrix, dist_coefs)
        for each in imgpoints2:
            cv2.circle(rgb_frame,(np.int16(each[0][0]),np.int16(each[0][1])),2,(0,0,255),2)
            linePtr1=(np.int16(each[0][0]),np.int16(each[0][1]))    
        imgpoints2, _ = cv2.projectPoints(np.array([[482.6,-368.1,-115.18]]), rvecs[-1], tvecs[-1], camera_matrix, dist_coefs)
        for each in imgpoints2:
            cv2.circle(rgb_frame,(np.int16(each[0][0]),np.int16(each[0][1])),2,(0,0,255),2)
            linePtr2=(np.int16(each[0][0]),np.int16(each[0][1]))
        cv2.line(rgb_frame,linePtr1,linePtr2,(0,255,0),2)
